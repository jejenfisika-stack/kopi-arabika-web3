...
// ============================================================
// Menu LAB — kumpulan laboratorium interaktif.
// Dipisah dari Pusat Belajar (Belajar.js) karena satu halaman berisi
// sebelas bagian sudah terlalu panjang, dan karena keduanya menuntut
// kegiatan berbeda: di sini mahasiswa MENGUTAK-ATIK lalu mengamati,
// sedangkan di Belajar mereka membaca, merenungkan, dan diukur.
//
// Kamus teks dwibahasa tetap tinggal di Belajar.js dan diimpor dari sana
// agar tidak ada dua sumber teks yang bisa saling menyimpang.
// ============================================================
import { useState, useEffect, useRef } from 'react'
import { L } from './Belajar'
...
export default function Lab({ lang }) {
  const t = L[lang] || L.id

  return (
    <section className="section" id="lab">
      <div className="section-head"><span className="ic">🧪</span><h3>{t.labTitle}</h3></div>
      <p className="section-sub">{t.labSub}</p>

      <nav className="learn-nav" aria-label={t.navHead}>
        <span className="learn-nav-lbl">{t.navHead}</span>
        <div className="learn-nav-list">
          {t.navLab.map(([id, label]) => (
